      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-stone-200 dark:border-stone-800">
        <div>
          <h1 className="text-2xl sm:text-4xl font-serif font-bold text-ink-900 dark:text-ink-50 flex items-center space-x-2.5">
            <History className="w-7 h-7 sm:w-8 h-8 text-chinese-ochre" />
            <span>最近浏览足迹</span>
          </h1>
          <p className="text-xs sm:text-sm text-ink-400 mt-1">
            记录您在此设备上研读过的诗篇，最多保留 100 条记录
          </p>
        </div>
        ...
        {history.length > 0 && (
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-medium flex items-center space-x-1.5 transition-colors self-start sm:self-auto"
          >
        ...
        <div className="space-y-8">
          {groups.map((group) => (
            <div key={group.label} className="space-y-3">
              ...
              {/* Items List */}
              <div className="bg-white dark:bg-chinese-nightCard border border-stone-200/80 dark:border-chinese-nightBorder rounded-2xl divide-y divide-stone-100 dark:divide-stone-800 shadow-oriental overflow-hidden">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-xs text-ink-400 font-serif flex-shrink-0">
                          〔{item.dynasty?.name || '古'}〕{item.author?.name || '佚名'}
                        </span>
                      </div>

                      {item.snippet && (
                        <p className="text-xs text-ink-500 dark:text-ink-400 font-serif truncate mt-1">
                          « {item.snippet} »
                        </p>
                      )}
                    </Link>
                    ...
                    <div className="flex items-center space-x-3 flex-shrink-0">
                      <span className="text-[11px] text-ink-400 font-sans hidden sm:inline">
                        {formatRelativeTime(item.viewedAt)}
                      </span>

                      <button
                        onClick={() => removeHistory(item.id)}
                        className="p-1.5 text-stone-400 hover:text-rose-500 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors"
                        title="删除该条记录"
                      >
                        <X className="w-4 h-4" />
                      </button>
